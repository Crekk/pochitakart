...
kart.held_item = 'chainsaw' #testing
...
# ----- AI Karts -----
ai_karts = []
ai_textures = ['pochita_green.png', 'pochita_blue.png', 'pochita_purple.png', 'pochita_green.png', 'pochita_green.png', 'pochita_green.png', 'pochita_green.png',  'pochita_green.png']
ai_spawn_offsets = [
...
def show_results_screen():
    global results_shown, countdown_done
...
def update():
    ...
    # Player flight control with Space
    if held_keys['space']:
        kart.vertical_speed = 20
    ...
    for ai in ai_karts:
        ...
        # --- AI Item Use: Reze ---
            ...
            if targets_in_range or random.random() < 0.2 * time.dt:
                print(f"[AI ITEM] {ai.texture} used reze!")
                ...
                for target in targets_in_range:
                    print(f"{getattr(target, 'texture', 'player')} got blasted!")
        ...
        # --- AI Item Use: Monkima ---
            ...
            if random.random() < 0.5 * time.dt:
                print(f"[AI ITEM] {ai.texture} used monkima!")
    ...
    if lap_trigger.intersects(kart_hitbox).hit:
        if not has_crossed and speed > 2 and kart.passed_middle:
            ...
            current_lap_time = 0
            if lap_count >= 3 and "Player" not in finish_order:
